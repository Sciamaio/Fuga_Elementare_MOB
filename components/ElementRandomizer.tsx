    ...
    return (
        <div className="flex flex-col items-center justify-center h-full bg-grid-cyan-500/10 p-8 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
                 <h1 className="font-orbitron text-5xl font-black text-white uppercase tracking-widest text-shadow-neon-cyan">
                    Generazione Parametri Missione
                </h1>
                 <p className="font-orbitron text-2xl text-cyan-300 mt-2 text-shadow-neon-cyan-sm animate-pulse">
                    Selezione elementi in corso...
                </p>

                <div className="mt-12 grid grid-cols-4 gap-4">
                    { (displayedSymbols.length > 0 ? displayedSymbols : Array(8).fill('?')).map((symbol, index) => (
                        <div key={index} className="w-24 h-24 bg-slate-800/50 border-2 border-slate-700 rounded-lg flex items-center justify-center">
                            <span className="font-orbitron text-4xl font-bold text-cyan-400">{symbol}</span>
                        </div>
                    ))}
                </div>
            </div>
